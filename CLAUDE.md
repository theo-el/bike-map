...
- **Frontend**: static site, no framework, no build step. `index.html` / `app.js` / `config.js` / `style.css`. MapLibre GL JS + OpenFreeMap vector tiles (free, no API key; style switchable via `?style=liberty|positron`, default liberty). Deployed on GitHub Pages. Vehicles render as a GPU circle layer (not DOM markers — thousands of them would lag on a phone); destination/my-location stay as regular `maplibregl.Marker` DOM elements since there's only one of each.
